    ...
    with open(os.path.join(args.data_dir, filename), 'r') as lines:
        if args.doc_level:
            ...
            for i, line in enumerate(lines):
                ...
                # need to add a check here in case the line starts with sentence symbols (we don't want context to be empty, which it will be otherwise)
                shift_up = next(idx for idx, tok in enumerate(sentences) if tok.strip()) # after strip whitespace eval to False, so gets first "real" token
                split_point = args.len_context + shift_up # if first token is valid shift up is 0
